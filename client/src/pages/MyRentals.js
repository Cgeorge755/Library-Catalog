...
function MyRentals() {
  ...
  const handleRenew = async (rentalId) => {
    try {
      const response = await API.put(`/rentals/${rentalId}/renew`);
      const { new_due_date, renewals_remaining, additional_cost } = response.data;
      alert(
          `Renewed successfully!\n` +
          `New due date: ${new_due_date}\n` +
          `Additional cost: $${additional_cost.toFixed(2)}\n` +
          `Renewals remaining: ${renewals_remaining}`
      );
      fetchRentals();
    } catch (error) {
      alert(error.response?.data?.detail || "Could not renew rental");
    }
  };
        ...
        {rentals.length === 0 ? (
            ...
            <div className="card-grid">
              {rentals.map((rental) => {
                ...
                const renewalsUsed = rental.renewals || 0;
                const renewalsLeft = 2 - renewalsUsed;
                    ...
                    >
                      ...
                      {!rental.returned && (
                          <>
                            ...
                            {/* Renewals remaining indicator */}
                            <p>
                              <strong>Renewals:</strong>{" "}
                              <span style={{ color: renewalsLeft === 0 ? "#dc2626" : "#16a34a" }}>
                        {renewalsLeft} of 2 remaining
                      </span>
                            </p>

                            {daysLeft <= 2 && daysLeft >= 0 && (
                                <p className="due-soon-text">
                                  Reminder: Return or renew this book soon
                                </p>
                            )}
                            ...
                            <div className="card-actions">
                              <button
                                  ...
                                  title={renewalsLeft === 0 ? "Maximum renewals reached" : `Extends due date by ${rental.rental_days} days`}
                              >
                                Renew
                              </button>
